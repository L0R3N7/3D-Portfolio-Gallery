...
Die \emph{Seperation of Concerns} wird bei diesem Pattern (siehe \ref{glos:pattern}) durch die Aufteilung der Applikation in \emph{Model, View und Controller}  realisiert. Dadurch ist es möglich, sich auf einen Aspekt der Implementierung zu fokussieren. 
...
Angular ist ein hoch anpassbares Framework. Bei der Verarbeitung von asynchronen Prozessen und Events kann Angular um die Library RxJS erweitert werden. Bei der Paketierung und beim Update von geändertem Code wird der Entwickler von der Libary Webpack unterstützt. \cite{AngularWebpack, AngularRxJs} 
...
\begin{compactitem}
    \item Sie vereinfachen und beschleunigen den Entwicklungsprozess. Typische Web-Komponenten sind vordefinierte, der Entwickler muss diese nicht mehr implementieren und erspart sich Zeit. \cite{BestCSSFrameworksin2022}
...
\begin{compactitem}
    ...
    \item Reibungslos: Angular Material ist einzig und allen für Angular als UI-Library entwickelt worden und lässt sich dadurch mit Angular leicht integrieren.\cite{JavaPointAngularMaterial}
\end{compactitem}
...
\emph{mat-icon} wird dafür verwendet, die Icons auf der Single-Page-Application anzuzeigen. Angular Material besitzt eine Icon-Library. Wenn \emph{mat-icon} verwendet wird, wird durch den Inhalt des \emph{mat-icon}-Tags das Icon dynamisch aus der Icon-Library geladen.
...
\begin{wrapfigure}{r}{0.3\textwidth}
  ...
  \end{center}
\end{wrapfigure}
Bootstrap ist ein kostenloses Open-Source-Frontend-Framework. Es fungiert als eine Art Bibliothek mit zahlreichem HTML sowie CSS Gestaltungsvorlagen. Es bietet mehrere Komponenten an. Ziel von Bootstrap ist es den*der Entwickler*in das Umsetzen von interaktive und responsive Webseiten zu erleichtern. \cite{BestCSSFrameworksin2022}
...
\begin{wrapfigure}{r}{0.3\textwidth}
    ...
    \end{center}
\end{wrapfigure}
\emph{Three.js} ist eine JavaScript Library für die Darstellung von 3D-Grafiken im Web. Für die 3D-Darstellung nutzt \emph{Three.js} \emph{WebGL}  (mehr dazu im nächsten Abschnitt \hyperref[ch::webgl]{WebGL}), ein Low-Level-Framework. \emph{WebGL} hat eine hohe Komplexität. \emph{Three.js} bietet eine Abstraktion von \emph{WebGL} , um Anwendungen für 3D-Webanwendungen für Entwickler zugänglicher zu machen.
\emph{Three.js} regelt Systeme wie die 3D-Szene, Lichter, Schatten, Materialien, Texturen und 3D-Matrix-Rechnungen, deren Umsetzung mittels \emph{WebGL}  sehr aufwändig wäre. \cite{ThreeJsFund}

\begin{figure} [h t]
  \centering
  \includegraphics[scale=0.5]{pics/threejs-structure.png}
  \caption{Die Struktur von \emph{Three.js} \cite{ThreeJsFund}}
  \label{fig:tech:front:threejsstructure}
\end{figure}

In \emph{Three.js} werden Geometrie, Objekte und Materialien verbunden, um ein 3D-Objekt zu erstellen. 
...
\paragraph{Angular Three - Abgebrochene Entwicklung}
